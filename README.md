...
## :star: Key Architecture: MVVM + Combine
### 본 프로젝트는 데이터의 흐름을 단방향으로 관리하고, 상태 변경에 따른 UI 업데이트를 자동화하기 위해 Combine을 적극 활용했습니다.



## Why Combine? 
* Native Framework: 외부 라이브러리 의존성 없이 Apple의 퍼스트 파티 프레임워크를 사용하여 프로젝트의 경량화와 안정성을 유지했습니다.
...
## :star: Technical Challenges & 트러블슈팅

### 개발 과정에서 직면한 성능 문제를 해결하기 위해 다음과 같은 최적화를 진행했습니다.

###  :ballot_box_with_check: WebSocket 리소스 과부하 최적화 
...
### :ballot_box_with_check: cell favoriteButton 셀 재사용 버그

* ✅해결: 셀 재사용시 재사용셀을 리셋해주지않아 중복클릭되는 오류가 있어 prepareForReuse()코드를 cell 안에 넣어줌으로써 해결.


### :ballot_box_with_check: CoinDataManager로 데이터 흐름정리 - 단일 신뢰 원칙
...
###  :ballot_box_with_check: UserDefaults 선택 근거

* ✅ 이 프로젝트의 즐겨찾기 기능은 코인 ID나 심볼과 같은 단순한 문자열 데이터를 관리합니다. 따라서 복잡한 데이터베이스(CoreData, Realm)를 도입하기보다, 가볍고 빠른 UserDefaults를 사용하는게 적합하다고 판단했습니다.
